...
module.exports = class OrderService {
  ...
  async delete(order) {
    console.log("sucess1");
    return await OrderModel.deleteOne({ userId: ObjectId(order) });
  }
  ...
  // all user order 
  async getAllOrder() {
    return await OrderModel.aggregate([{ $match: {} },
    {$group:{_id:'$userId',totalprice: {$sum:'$totalPrice'},status:{$push:'$status'}}},
  {$project:{user:'$_id',totalPriceSpend:'$totalprice',
  status:'$status'
  }}]);
  }
